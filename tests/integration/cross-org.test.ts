...
describe.skipIf(!hasIntegrationEnv())('cross-org collaboration (D2 mini)', () => {
  ...
  it('Alice creates, invites Bob, Bob joins and sees history', async () => {
    ...
    await alice.dispatcher.invoke('channel_send', { channel_id: cid, content: 'alice msg 1' });
    await alice.dispatcher.invoke('channel_invite', { channel_id: cid, address: bob.address });

    // Bob joins
    const j = (await bob.dispatcher.invoke('channel_join', { channel_id: cid })) as {
      history_count: number;
    };
    expect(j.history_count).toBeGreaterThanOrEqual(1);

    // Bob sends, Alice reads it back and verifies
    await bob.dispatcher.invoke('channel_send', { channel_id: cid, content: 'bob msg 1' });
    const hist = (await alice.dispatcher.invoke('channel_history', {
      channel_id: cid,
    })) as {
      messages: Array<{
        message_id: string;
    ...
    const v = (await alice.dispatcher.invoke('identity_verify', {
      channel_id: cid,
